...
export default async function HotelDetailPage({ params, searchParams }: PageProps) {
  ...
  // Fetch hotel detail + rates in parallel (equivalent to PHP curl_multi)
  ...
  // Inject resolved room names and sort by price
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const descShort = rawDesc.slice(0, 500)
                ...
                <div className="card card-compact" style={{ marginBottom: 16 }}>
                  ...
                  {importantInfo && (
                    <div className="policy-important">
                      ...
                      <div className="policy-important-text">{importantInfo.slice(0, 300)}{importantInfo.length > 300 ? '...' : ''}</div>
                    </div>
                  )}
                </div>
              </>
